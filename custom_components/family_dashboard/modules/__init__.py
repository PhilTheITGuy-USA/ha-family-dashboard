...
The registry of available per-member-toggleable features lives in `const.py`'s `FEATURES`
dict (key -> display name, platforms, default_selected, implemented). Each module is a
subpackage here (`modules/<key>/`) that should contain:
  ...
  - (optional) a config-flow step class, for modules that need extra input beyond the
    ...
    `config_flow.py`'s `async_step_features`/`async_step_link_users`, gated on at least one
    roster member having that module's key in their own `features` list.
...
whatever's in `FEATURES`.

New module checklist (e.g. adding Meals later): create `modules/meals/`, add its entity
platform file(s) + top-level shim(s), add an entry to `const.py`'s `FEATURES`, add any
